...
##根据value值的大小设置连线的颜色
circos.snplink <- function(data, ylim=c(0,1), link.col=rainbow(3),
  ...
  for (i in seq_len(nrow(data))) {
    colorkey <- cut(data[i, 7], seq(legend.min, legend.max, 0.05), labels=link.col)
...
circos.legendlink <- function(x=0.8, y=0.5, legend.limits=c(min,max),legend.bin=1, legend.col=legend.col,
  ...
  text(x=rep(x+0.085, legend.rect/2+1), y=seq(y, y+0.05*(legend.rect/2), 0.05*legend.bin),
       labels = c(seq(legend.min, legend.max, 0.1*legend.bin)),
