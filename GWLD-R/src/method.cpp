...
double MIC(arma::Col<int> &g1, arma::Col<int> &g2)
{
    ...
    arma::Col<double> rowh = a.transform([](double val)
                                         { return (val == 0.0 ? 0.0 : -val * log2(val)); });
    arma::Row<double> colh = b.transform([](double val)
                                         { return (val == 0.0 ? 0.0 : -val * log2(val)); });
    arma::Mat<double> tabh = freq.transform([](double val)
                                            { return (val == 0.0 ? 0.0 : -val * log2(val)); });
    // 计算位点g1的信息熵
